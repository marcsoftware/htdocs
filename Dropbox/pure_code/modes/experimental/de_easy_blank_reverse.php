...
function NewQuiz (text,handle) {
    ...
    this.colors = []; // holds the higlihed chars for smarCompare()
     Quiz.BLOCK = 3; // number of lines the user enters at once
    ...
    alert(text.replace(/\t/g,'='));
        ...
        this.codes=[];
        for(var i =0;i<new_text.length;i+=2){ //PROBLEM i = 0 in this file but in MC it has to be i=1;
          this.codes.push(new_text[i+1]+'\t'+new_text[i]);
        }
 ...
 this.gaveup = function(){
      ...
      targets_array = targets.split('<br/>');
       key = targets_array[Quiz.page_score];
        ...
        document.getElementById('missed').innerHTML='<h6>'+missed_word+'</h6>';
        document.getElementById('missed').innerHTML+='<br/>'+targets_array+'<br/>';
        document.getElementById('missed').innerHTML+=original_terms;
