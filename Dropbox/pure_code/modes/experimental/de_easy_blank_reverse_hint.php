...
function NewQuiz (text,handle) {
    ...
    this.colors = []; // holds the higlihed chars for smarCompare()
     Quiz.BLOCK = <?php echo $_GET['block']?>; // number of lines the user enters at once
    ...
    this.create = function(id) {
        ...
        //reverse the text so that user types lefthand side.
        var new_text=text.replace(/\t+/g,'\t');
        new_text=new_text.replace(/\n+/g,'\n');
        new_text=new_text.replace(/\,/g,'¸');
        new_text=new_text.split(/[\t\n]/g);
        
        new_text=new_text.filter(function(n){ return n != undefined });   
        
        this.codes=[];
        for(var i =0;i<new_text.length;i+=2){ // TODO: reoccuring bug: i = 1, but if i delete 1st comment do i need to make i=0?  

          this.codes.push(new_text[i]+'\t'+new_text[i+1]);
        }
        ...
        //this.handle.innerHTML="<p></p> <p></p><input id='userbox' type='text'></input> " ;//make the place holder elements
        this.handle=document.getElementById('here');
       ...
       try{
        ...
        var list = this.handle.getElementsByTagName('input');
        var obj= this;
        var ans = this.getCurrentLine();
        this.scrambleDisplay();
        list[0].addEventListener("keyup", function(){
          obj.check(this.value);
        }); 
...
function playSound() {
  ...
  function compare(x,y){
    ...
    this.scrambleDisplay = function(input='') {
       ...
       list[1].innerHTML="<h6>"+terms+"</h6>"; // hints.
...
<pre id='file'></pre>
<div id='here'>
<p></p> <p></p><input id='userbox' type='text'></input>
</div>
